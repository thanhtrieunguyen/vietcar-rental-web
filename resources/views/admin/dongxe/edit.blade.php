            ...
            <div class="w-full max-w-4xl">
                <div class="bg-white rounded-lg shadow-sm border-0 p-6">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 py-3 mb-4">
                        <div>
                            <h5 class="text-xl font-bold">Cập nhật dòng xe <small class="text-gray-500 font-normal">{{ $dongXe->tendongxe }}</small></h5>
                        </div>
                        <div>
                    ...
                    <form action="{{ route('dongxe.update', $dongXe->iddongxe) }}" class="mb-3" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="mb-4">
                            <label for="tenDongXe" class="block text-sm font-semibold text-gray-700 mb-2">
                            ...
                            <input type="text" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary {{ $errors->has('tendongxe') ? 'border-red-500' : 'border-gray-300' }}" 
                                id="tenDongXe" name="tendongxe" placeholder="Nhập tên dòng xe" value="{{ $dongXe->tendongxe }}">
